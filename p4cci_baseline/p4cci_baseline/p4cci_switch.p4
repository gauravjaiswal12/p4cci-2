...
control MyIngress(inout headers_t hdr,
    ...
    // Key: 3-tuple (src_ip, dst_ip, dst_port) — srcPort is excluded because
    // iperf3/TCP client ports are ephemeral and unknown at rule-insertion time.
    // (src_ip, dst_ip, dst_port) uniquely identifies each flow in our topology.
...
control MyEgress(inout headers_t hdr,
    ...
    // Traffic separation via BMv2 priority queue assignment.
    // standard_metadata.priority is set here; actual BW enforcement
    // is done by TC-HTB on the Linux interface (s1-eth3) using DSCP marks.
    ...
    apply {
        if (hdr.tcp.isValid()) {
            // Step 1: Assign BMv2 priority queue
            queue_assignment.apply();

            // Step 2: Mark DSCP based on CCA class.
            // Linux TC-HTB on s1-eth3 uses this DSCP field to
            // classify packets into guaranteed-bandwidth queues.
            //   cca_class=1 (loss-based CUBIC/Reno): DSCP=10 -> diffserv=0x28
            //   cca_class=2 (model-based BBR):       DSCP=18 -> diffserv=0x48
            // The IPv4 checksum is recomputed in MyComputeChecksum.
            if (meta.cca_class == 1) {
                hdr.ipv4.diffserv = 0x28; // AF11
            } else if (meta.cca_class == 2) {
                hdr.ipv4.diffserv = 0x48; // AF21
            }
        }
    }
}
